...
| SW1〜SW5 | D7,D8,D9,D12,D13 | 各スイッチ片側（もう片側→GND共通） |
...
```
SW1→D7 / SW2→D8 / SW3→D9 / SW4→D12 / SW5→D13
各スイッチのもう片側 → GND（共通）
```

`INPUT_PULLUP` を使うので**抵抗は不要**（押していないとき HIGH、押すと LOW）。
タクトスイッチは**対角の2足**を使うこと。サーボ用の D5/D6 は避けている。

> [!NOTE]
> D13 は Nano/UNO のオンボード LED と直結。`INPUT_PULLUP` で読めるが、
> SW5 だけ不安定なら別の空きピン（A1 など digitalRead 可能なピン）へ移す。
...
| `+` `-` | 音量調整 |
| `?` | ファイル数・音量の確認 |
| **SW1〜SW5**（物理） | 押した瞬間に対応番号を発火（シリアルと併用可） |
...
- 物理スイッチ SW1〜SW5 で PC なし発火（押した瞬間・デバウンス付き）に対応
