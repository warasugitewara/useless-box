...
- [ ] **🔢 実行中 case の表示モニター**
  - 動作中のバリエーション番号（1〜5）を数字表示するモジュールを追加
  - 候補:
    - **TM1637 4桁7セグ** — 数百円・2ピン(CLK/DIO)・数字表示に特化。最有力
    - SSD1306 0.96" OLED (I2C) — A4/A5 使用・文字も出せるが今回はオーバースペック気味
  - 空きピン: `D2`〜`D4` / `D7`〜`D9` / `A1`〜`A5`（A0 はランダムシード用に未接続維持）
