...
<details>
<summary><b>🐛 キャタピラー走行機構（別プロジェクトとして分離）</b></summary>

<br>

チームメンバーがハード側で実装中。**ESP32 によるラジコン方式の独立系統**として作るため、
本機の回路には統合しません。

### なぜ分離したか

- ブラシ付きDCモーターは強いノイズ源。電源とGNDを共有すると、サーボの誤動作や
  DFPlayer 導入後のスピーカーノイズの原因になり得るが、**別系統にすれば問題ごと消える**
- 3系統目の電源やモータードライバが不要になり、本機は2電源構成のまま維持できる
- 制御ピンを消費しないため、`D7`〜`D9`・`D13` が丸ごと空いたままになる
...
| 通信方式 | **ESP-NOW**（送信機・受信機ともに ESP32。ルーター不要・低遅延） |
| 操作部 | ジョイスティックモジュールを配線して自作コントローラ化 |
| モータードライバ | **TOSHIBA TA7291P** |

### 検討時に洗い出した注意点

- **ESP32 は 3.3V ロジック**。TA7291P の Vcc は 3.3V で動くのでレベル変換は不要
- **`Vs`（モーター電源）は `Vref` より高い必要がある** — Vref に 3.3V PWM を入れるなら
  Vs は余裕をもって 6V 以上に。バイポーラ出力段ぶんの電圧降下もある
- **TA7291P は 1個で 1モーター**。左右2モーターなら **2個必要**
- TA7291P は新規製造中止の方向のため、予備は早めに確保しておく
- **ADC2 は WiFi 有効時に使えない**（ESP-NOW は WiFi 無線を使う）。
  ジョイスティックのアナログ2軸は **ADC1（GPIO 32/33/34/35/36/39）** に繋ぐこと
- ジョイスティックの電源は **3.3V から取る**（5V だと ADC の入力上限を超える）
- 通信途絶時にモーターを止める **フェイルセーフ**（受信タイムアウト → 停止）を必ず入れる
- ESP32 Arduino core 3.x で LEDC(PWM) の API が変更済み。
  古い `ledcSetup()`/`ledcAttachPin()` 形式の記事はそのまま使えない

> [!NOTE]
> このキャタピラー機構は本リポジトリの管理対象外です。実装が始まったら別リポジトリを立てます。
