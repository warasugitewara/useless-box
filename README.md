...
- [ ] **🔢 実行中 case の表示 — 7セグLED + シフトレジスタ**
  - 動作中のバリエーション番号（1〜5）を7セグLEDに数字表示
  - 使用部品:
    - **7セグLED: PARA LIGHT A-552SRD**（2桁品・**アノードコモン**）— DIG.1のみ使用
    - **シフトレジスタ: 74HC595（UTC U74HC595A）** — Arduino 3ピンで8出力を駆動
  - 74HC595 を挟む理由: 7セグ直結だと 7ピン消費するが、シフトレジスタ経由なら **3ピン**（DS/SHCP/STCP）で済む
  - 接続予定: `D3`(DS/SER) / `D4`(SHCP/SRCLK) / `D7`(STCP/RCLK) → 74HC595 → 各セグメント(a〜g)
  - 電流制限: 74HC595 は合計 ±70mA 制約。各セグメントに **470Ω** を入れ 1セグ約8mA に抑える（"8"全点灯でも合計内に収める）
  - アノードコモンのため 74HC595 出力 **L で点灯**（コモンピン14を電源へ）
